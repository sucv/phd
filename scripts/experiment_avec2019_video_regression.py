...
def main_avec2019(tc="all", vc="all", gpu_index=None, cpu_thread_number=None):
    ...
    folder_arranger = AVEC19ArrangerNPY(config)
    train_dict, validate_dict = folder_arranger.make_data_dict(
        train_country=config['train_country'], validate_country=config['validate_country'])
